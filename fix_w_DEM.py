...
plot_floating_cutoff=1 # plot all segments with a reach floating higher than cutoff
plot_incised_cutoff=50 # plot all segments
...
# profiles of worst floating segments
df=pd.DataFrame(Diffs['DIF'],index=Diffs['MSEG'])
#nreaches=int(round(len(df)*percentile))
Floats=df.sort(columns=0,ascending=True)
Floats=Floats[0][Floats[0]<0]
Floatsegs=Floats.groupby(Floats.index).max() # group floating reaches by segment, only retain reach with largets float difference
Floatsegs.sort()
floats=list(Floatsegs[Floatsegs<-plot_floating_cutoff].index) # filter top floating segments to exclude those below the above cutoff
Plot_segment_profiles.plot_profiles(floats,seg_distdict,[TOPNEWdict,STOP2dict],['model top','streambed top post-fix_w_DEM'],'Segments_floating_by_more_than_%s.pdf' %(plot_floating_cutoff))

# profiles of worst incised
df=pd.DataFrame(Diffs['DIF'],index=Diffs['MSEG'])
Incised=df.sort(columns=0,ascending=False)
Incised=Incised[0][Incised[0]>0]
Incisedsegs=Incised.groupby(Incised.index).max() # group floating reaches by segment, only retain reach with largets float difference
Incisedsegs.sort()
incised=list(Incisedsegs[Incisedsegs>plot_incised_cutoff].index)
Plot_segment_profiles.plot_profiles(incised,seg_distdict,[TOPNEWdict,STOP2dict],['model top','streambed top post-fix_w_DEM'],'Segments_incised_by_more_than_%s.pdf' %(plot_incised_cutoff))
