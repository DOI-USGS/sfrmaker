...
comidseen=dict()
ofpw = open('finalize_SFR2.warnings','w')
...
for cell in cellrows:
    ...
    if cell.LengthFT>0.000001:
        ...
        localslope=0.
    if localslope < 0.:
        print 'COMID: %d --> min elevation > max elevation' %int(cell.COMID)
        ofpw.write('COMID: %d CELLNUM: %d --> min elevation > max elevation\n' %(int(cell.COMID),int(cellnum)))
...
for cellnum in row:
    for i in range(0,len(comid[cellnum])):
        print cellnum, i
        if comid[cellnum][i] not in arbolate.keys():
            arbolate[comid[cellnum][i]] = 9999999
            missingCOMIDS.append(comid[cellnum][i])
            if arbfailflag == 0:
                arbfixfile = open(fixarb,'w')
                arbfailflag += 1
            arbfixfile.write('Need to fix width and arbolate: COMID = %d, cellnum = %d\n' %(int(comid[cellnum][i]),int(cellnum)))
            estwidth[cellnum].append(9999999999) # failed on arbolate so arbitrary junk width        
        else:
    ...
    for i in range(0,len(comid[cellnum])):
        ...
        if comidcell not in missingCOMIDS:
            printstring=(cellnum,
                     row[cellnum],
                     column[cellnum],
...
ordering=open(ORDER,'r').readlines()
junk = ordering.pop(0) # skip the header
